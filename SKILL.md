...
MainActivity                 ← 入口，AlgaeTheme + TrainViewModel
  └─ MainScreen              ← Scaffold + 3 标签页 + 2 个全屏 overlay
       ├─ TrainTab           ← 训练页：当前书卡片 + 今日任务打卡区 + 闪卡
       ├─ ErrorBookTab       ← 错题集：模式切换 + 列表 + 详情查看
...
- **无内存缓存** — 按需从 assets 解码，629+146 张图按需加载策略合理
