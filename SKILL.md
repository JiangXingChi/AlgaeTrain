...
- **打卡条件（V0.6.3 派生式）**：有未学新卡 → 新卡组全部「认识」；无新卡有到期复习 → 复习组全部完成；两者皆无（学完书错峰空档日）→ 视为达标。撤销使进度回落会回收当天自动打卡（按书记账，含重置冻结）
...
// MainScreen.kt
private const val VERSION = "V0.6.6"
```
...
- **打卡不回滚** → V0.6.3 起改为派生对齐：undo 使进度回落时回收当天自动打卡；跨书/重置由按书记账（`checkin_done_<day>` / `checkin_sealed_<day>`）与 `sealIdleBooks()` 空档日兜底保证不误删
